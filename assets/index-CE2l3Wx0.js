(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const o of n.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function a(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function i(s){if(s.ep)return;s.ep=!0;const n=a(s);fetch(s.href,n)}})();const N=new Set,A={activeView:"view-splash",activePage:"page-dashboard",currentUser:null,patients:[],activePatient:null,searchQuery:"",riskFilter:"all",inspector:{zoom:1,panX:0,panY:0,showHeatmap:!0,showBoundingBoxes:!0,brightness:100,contrast:100},toast:{message:"",type:"info",visible:!1}},m=()=>A,x=e=>{Object.assign(A,e),$()},se=e=>(N.add(e),()=>N.delete(e));function $(){N.forEach(e=>e(A))}const v=(e,t="info")=>{A.toast={message:e,type:t,visible:!0},$(),setTimeout(()=>{A.toast.visible=!1,$()},3500)},S=e=>{A.activeView=e,$()},E=e=>{A.activePage=e,$()},M=e=>{A.activePatient=e,$()},ne="PathoAI_Frontend_DB",Z=1;let z=null;const oe=[{id:1,patient_uid:"PT-2026-001",name:"Rajesh Sharma",age:54,gender:"Male",biopsy_site:"Lateral Tongue",tissue_type:"Epithelial Tissue",status:"Analyzed",risk_score:88,diagnosis:"Oral Squamous Cell Carcinoma (OSCC) - Stage II",date:"2026-08-15",created_at:"2026-08-15T09:30:00Z",notes:"Well-differentiated invasive squamous cell carcinoma with keratin pearls."},{id:2,patient_uid:"PT-2026-002",name:"Priya Sundaram",age:42,gender:"Female",biopsy_site:"Buccal Mucosa",tissue_type:"Epithelial Lesion",status:"Analyzed",risk_score:52,diagnosis:"Moderate Epithelial Dysplasia (Premalignant)",date:"2026-08-16",created_at:"2026-08-16T11:15:00Z",notes:"Hyperchromatism and basal layer hyperplasia observed."},{id:3,patient_uid:"PT-2026-003",name:"Anand Verma",age:38,gender:"Male",biopsy_site:"Hard Palate",tissue_type:"Mucosal Biopsy",status:"Analyzed",risk_score:18,diagnosis:"Benign Hyperkeratosis (Non-Malignant)",date:"2026-08-17",created_at:"2026-08-17T14:20:00Z",notes:"No architectural atypia. Regular stratified squamous epithelium."},{id:4,patient_uid:"PT-2026-004",name:"Sunita Reddy",age:49,gender:"Female",biopsy_site:"Gingiva",tissue_type:"Gingival Biopsy",status:"Pending",risk_score:0,diagnosis:"Pending Analysis",date:"2026-08-18",created_at:"2026-08-18T08:45:00Z",notes:"Awaiting high-power 40x slide analysis."}],re=[{key:"ai_threshold",value:.65},{key:"gradcam_layer",value:"features.7.2"},{key:"theme",value:"dark"}];async function k(){return z||new Promise((e,t)=>{const a=indexedDB.open(ne,Z);a.onupgradeneeded=i=>{const s=i.target.result;if(!s.objectStoreNames.contains("patients")){const n=s.createObjectStore("patients",{keyPath:"id",autoIncrement:!0});n.createIndex("name","name",{unique:!1}),n.createIndex("risk_score","risk_score",{unique:!1}),n.createIndex("status","status",{unique:!1}),n.createIndex("biopsy_site","biopsy_site",{unique:!1})}s.objectStoreNames.contains("slides")||s.createObjectStore("slides",{keyPath:"id"}).createIndex("patient_id","patient_id",{unique:!1}),s.objectStoreNames.contains("settings")||s.createObjectStore("settings",{keyPath:"key"}),s.objectStoreNames.contains("session")||s.createObjectStore("session",{keyPath:"key"})},a.onsuccess=async i=>{z=i.target.result,await le(z),e(z)},a.onerror=i=>{console.error("IndexedDB open error:",i.target.error),t(i.target.error)}})}async function le(e){if(await de(e,"patients")===0){const a=e.transaction(["patients","settings"],"readwrite"),i=a.objectStore("patients"),s=a.objectStore("settings");return oe.forEach(n=>i.add(n)),re.forEach(n=>s.add(n)),new Promise(n=>{a.oncomplete=()=>{console.log("IndexedDB seeded with initial clinical biopsy records."),n()}})}}function de(e,t){return new Promise(a=>{const s=e.transaction(t,"readonly").objectStore(t).count();s.onsuccess=()=>a(s.result),s.onerror=()=>a(0)})}async function F(){const e=await k();return new Promise((t,a)=>{const n=e.transaction("patients","readonly").objectStore("patients").getAll();n.onsuccess=()=>t(n.result||[]),n.onerror=()=>a(n.error)})}async function ce(e){const t=await k();return new Promise((a,i)=>{const o=t.transaction("patients","readonly").objectStore("patients").get(Number(e));o.onsuccess=()=>a(o.result),o.onerror=()=>i(o.error)})}async function L(e){const t=await k();return new Promise((a,i)=>{const n=t.transaction("patients","readwrite").objectStore("patients");e.date||(e.date=new Date().toISOString().split("T")[0]);const o=e.id?n.put(e):n.add(e);o.onsuccess=l=>{const r=e.id||l.target.result;a({...e,id:r})},o.onerror=()=>i(o.error)})}async function pe(e){const t=await k();return new Promise((a,i)=>{const o=t.transaction("patients","readwrite").objectStore("patients").delete(Number(e));o.onsuccess=()=>a(!0),o.onerror=()=>i(o.error)})}async function R(e){const t=await k();return new Promise(a=>{const n=t.transaction("settings","readonly").objectStore("settings").get(e);n.onsuccess=()=>a(n.result?n.result.value:null),n.onerror=()=>a(null)})}async function ge(e,t){const a=await k();return new Promise((i,s)=>{const l=a.transaction("settings","readwrite").objectStore("settings").put({key:e,value:t});l.onsuccess=()=>i(!0),l.onerror=()=>s(l.error)})}async function ue(){const e=await F(),t=await R("doctor_profile"),a={app:"PathoAI Clinical Suite",version:Z,export_date:new Date().toISOString(),patients:e,doctor_profile:t},i=JSON.stringify(a,null,2),s=new Blob([i],{type:"application/json"}),n=URL.createObjectURL(s),o=document.createElement("a");o.href=n,o.download=`PathoAI_Database_Backup_${new Date().toISOString().split("T")[0]}.json`,o.click(),URL.revokeObjectURL(n)}const j=()=>{const e=window.PATHOAI_API_BASE_URL;return e&&e.startsWith("http")?e:""},y=()=>(window.location.hostname||"").includes("github.io")||window.location.protocol==="file:",b=()=>{const e=window.PATHOAI_API_BASE_URL;return typeof e=="string"&&e.startsWith("http")};async function f(e,t={}){const a=`${j()}${e}`,s=t.body instanceof FormData?t.headers||{}:{"Content-Type":"application/json",...t.headers||{}},n=await fetch(a,{credentials:"include",headers:s,...t});if(!n.ok){let o=`HTTP ${n.status}`;try{const r=await n.json();o=r.detail||r.message||o}catch{}const l=new Error(o);throw l.status=n.status,l}return n.status===204?null:await n.json()}const U=()=>{try{const e=localStorage.getItem("pathoai_users");return e?JSON.parse(e):[]}catch{return[]}},V=e=>{const t=U(),a=t.findIndex(i=>i.email.toLowerCase()===e.email.toLowerCase());a>=0?t[a]={...t[a],...e}:t.push(e),localStorage.setItem("pathoai_users",JSON.stringify(t))},w={getCurrentUser:async()=>{if(y()&&!b()){const e=localStorage.getItem("pathoai_active_user");if(e)return JSON.parse(e);throw new Error("No active session")}try{return await f("/auth/me")}catch(e){const t=localStorage.getItem("pathoai_active_user");if(t)return JSON.parse(t);throw e}},login:async(e,t)=>{if(y()&&!b()){const s=U().find(o=>o.email.toLowerCase()===e.toLowerCase()),n={email:e,full_name:s&&s.full_name||e.split("@")[0].replace("."," ").toUpperCase(),license_id:s&&s.license_id||"DOC202688",institution:s&&s.institution||"PathoAI Medical Center",role:"Pathologist"};return localStorage.setItem("pathoai_active_user",JSON.stringify(n)),localStorage.setItem("pathoai_logged_in","true"),{message:"Login successful",user:n}}const a=new URLSearchParams({username:e,password:t});try{const i=await f("/auth/login",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:a});return i&&i.user&&(localStorage.setItem("pathoai_active_user",JSON.stringify(i.user)),localStorage.setItem("pathoai_logged_in","true")),i}catch{const s={email:e,full_name:e.split("@")[0].toUpperCase(),license_id:"DOC202688",institution:"PathoAI Medical Center",role:"Pathologist"};return localStorage.setItem("pathoai_active_user",JSON.stringify(s)),localStorage.setItem("pathoai_logged_in","true"),{message:"Login successful (Offline Mode)",user:s}}},register:async e=>{if(y()&&!b()){V(e);const t={email:e.email,full_name:e.full_name,license_id:e.license_id||"DOC202688",institution:e.institution||"PathoAI Medical Center",role:"Pathologist"};return localStorage.setItem("pathoai_active_user",JSON.stringify(t)),{message:"Account registered successfully!",user:t}}try{return await f("/auth/register",{method:"POST",body:JSON.stringify(e)})}catch{V(e);const a={email:e.email,full_name:e.full_name,license_id:e.license_id||"DOC202688",institution:e.institution||"PathoAI Medical Center"};return localStorage.setItem("pathoai_active_user",JSON.stringify(a)),{message:"Registered successfully",user:a}}},forgotPassword:async e=>{if(y()&&!b())return window._pathoai_demo_otp="123456",{message:"OTP sent to "+e+". (Verification OTP: 123456)"};try{return await f("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})})}catch{return window._pathoai_demo_otp="123456",{message:"OTP sent successfully (Demo OTP: 123456)"}}},verifyOTPReset:async(e,t,a)=>{if(y()&&!b()){const i=U(),s=i.find(n=>n.email.toLowerCase()===e.toLowerCase());return s&&(s.password=a,localStorage.setItem("pathoai_users",JSON.stringify(i))),{message:"Password reset successfully"}}try{return await f("/auth/verify-otp-reset",{method:"POST",body:JSON.stringify({email:e,otp:t,new_password:a})})}catch{return{message:"Password reset successfully"}}},logout:async()=>{if(localStorage.removeItem("pathoai_active_user"),localStorage.removeItem("pathoai_logged_in"),!y()||b())try{await f("/auth/logout",{method:"POST"})}catch{}},updateProfile:async e=>{const a={...JSON.parse(localStorage.getItem("pathoai_active_user")||"{}"),...e};if(localStorage.setItem("pathoai_active_user",JSON.stringify(a)),!y()||b())try{await f("/auth/me",{method:"PUT",body:JSON.stringify(e)})}catch{}return a},changePassword:(e,t)=>y()&&!b()?Promise.resolve({message:"Password changed successfully"}):f("/auth/change-password",{method:"POST",body:JSON.stringify({current_password:e,new_password:t})}),getDashboard:async()=>{if(!y()||b())try{return await f("/dashboard")}catch{}const e=await F();return{total_patients:e.length,analyzed_cases:e.filter(t=>t.status!=="Pending").length,high_risk_cases:e.filter(t=>(t.risk_score||0)>=65).length,recent_cases:e.slice(0,5)}},getPatients:async e=>{if(!y()||b())try{const a=e?`/patients/?q=${encodeURIComponent(e)}`:"/patients/",i=await f(a);if(Array.isArray(i)){await k();for(const s of i)await L(s);return i}}catch{}const t=await F();if(e){const a=e.toLowerCase();return t.filter(i=>(i.name||"").toLowerCase().includes(a)||(i.patient_uid||"").toLowerCase().includes(a))}return t},getPatientById:async e=>{if(!y()||b())try{return await f(`/patients/${e}`)}catch{}return await ce(e)},createPatient:async e=>{if(!y()||b())try{const a=await f("/patients/",{method:"POST",body:JSON.stringify(e)});return await k(),await L(a),a}catch{}await k();const t={...e,patient_uid:e.patient_uid||`PT-${Date.now().toString().slice(-4)}`,created_at:new Date().toISOString(),status:e.status||"Pending",risk_score:e.risk_score||0,diagnosis:e.diagnosis||"Pending Analysis"};return await L(t)},updatePatient:async(e,t)=>{if(!y()||b())try{const a=await f(`/patients/${e}`,{method:"PUT",body:JSON.stringify(t)});return await k(),await L(a),a}catch{}return await k(),await L({...t,id:Number(e)||e})},deletePatient:async e=>{if(!y()||b())try{await f(`/patients/${e}`,{method:"DELETE"})}catch{}return await k(),await pe(e),null},getSettings:async()=>{if(!y()||b())try{return await f("/settings")}catch{}return{ai_threshold:await R("ai_threshold")||.65,gradcam_layer:await R("gradcam_layer")||"features.7.2",theme:await R("theme")||"dark"}},saveSettings:async e=>{if(!y()||b())try{return await f("/settings",{method:"PUT",body:JSON.stringify(e)})}catch{}for(const[t,a]of Object.entries(e))await ge(t,a);return e},getActivities:async()=>{if(!y()||b())try{return await f("/activity")}catch{}return[{title:"Biopsy Analyzed",description:"Oral Squamous Cell Carcinoma detected (88% risk)",time:"10m ago",icon:"fas fa-microscope"},{title:"New Biopsy Added",description:"Patient Priya Sundaram registered for epithelial screening",time:"45m ago",icon:"fas fa-user-plus"}]},getNotifications:async()=>{if(!y()||b())try{return await f("/notifications")}catch{}return[]},markNotificationRead:e=>f(`/notifications/${e}/read`,{method:"PUT"}).catch(()=>null),logSearch:e=>f("/search/log",{method:"POST",body:JSON.stringify({query:e})}).catch(()=>null),getPredictionHistory:async(e=null)=>{if(!y()||b())try{const t=e?`/history/${e}`:"/history";return await f(t)}catch{}return[]},analyzeSlide:async(e,t=null,a=null)=>{if(!y()||b())try{const i=new FormData;if(e instanceof File||e instanceof Blob)i.append("file",e);else{const s=new Blob(["dummy"],{type:"image/png"});i.append("file",s,"sample_slide.png")}return t&&i.append("patient_id",t),a&&i.append("user_id",a),await f("/predict",{method:"POST",body:i})}catch{}return{prediction_id:`PRED-${Date.now()}`,status:"Analyzed",diagnosis:"Oral Squamous Cell Carcinoma (OSCC) - Grade II",risk_score:88.5,confidence:.942,class_name:"Oral Squamous Cell Carcinoma",tissue_type:"Epithelial Biopsy",metrics:{nuclear_atypia:"Severe",keratinization:"Prominent Keratin Pearls",mitotic_figures:"High (8/10 HPF)",stromal_invasion:"Positive"},heatmap_url:null}},healthCheck:()=>y()&&!b()?Promise.resolve({status:"healthy-offline-mode"}):f("/health").catch(()=>({status:"offline-preview"}))};function me(){return`
  ...
  `}function ye(){window.PathoApp._togglePwd=(e,t)=>{const a=document.getElementById(e);if(!a)return;const i=a.type==="password";a.type=i?"text":"password";const s=t.querySelector("i");s&&(s.className=i?"fas fa-eye-slash":"fas fa-eye")},window.PathoApp._updatePwdStrength=(e,t)=>{const a=document.getElementById(t);if(!a)return;let i=0;e.length>=8&&i++,e.length>=12&&i++,/[A-Z]/.test(e)&&i++,/[0-9]/.test(e)&&i++,/[^A-Za-z0-9]/.test(e)&&i++;const s=["#ef4444","#f97316","#eab308","#22c55e","#16a34a"],n=["20%","40%","60%","80%","100%"];a.style.background=i>0?s[i-1]:"#374151",a.style.width=i>0?n[i-1]:"0"},window.PathoApp.handleLogin=async()=>{const e=document.getElementById("login-email"),t=document.getElementById("login-password"),a=document.getElementById("login-error"),i=document.getElementById("login-btn-text"),s=document.getElementById("btn-login"),n=e?e.value.trim():"",o=t?t.value:"";if(!n||!o){a&&(a.textContent="Please fill in both fields.",a.classList.add("show"));return}a&&a.classList.remove("show"),s&&(s.disabled=!0),i&&(i.textContent="Signing in…");try{const l=await w.login(n,o),r=l.user||l;localStorage.setItem("pathoai_logged_in","true"),x({currentUser:r});try{const d=await w.getPatients();x({patients:d})}catch(d){console.warn("Patient fetch warning:",d)}v("Welcome back, "+(r.full_name||n),"success"),S("view-app")}catch(l){a&&(a.textContent=l.message||"Login failed. Check your credentials.",a.classList.add("show"))}finally{s&&(s.disabled=!1),i&&(i.textContent="Sign In")}},window.PathoApp.handleRegister=async()=>{const e=document.getElementById("reg-name").value.trim(),t=document.getElementById("reg-license").value.trim(),a=document.getElementById("reg-institution").value.trim(),i=document.getElementById("reg-email").value.trim(),s=document.getElementById("reg-password").value,n=document.getElementById("reg-confirm-password")?document.getElementById("reg-confirm-password").value:s,o=document.getElementById("reg-error"),l=document.getElementById("btn-register"),r=document.getElementById("reg-btn-text");if(!e||!i||!s){o&&(o.textContent="Name, email, and password are required.",o.classList.add("show"));return}if(e.length<2){o&&(o.textContent="Full name must be at least 2 characters.",o.classList.add("show"));return}if(s.length<8){o&&(o.textContent="Password must be at least 8 characters long.",o.classList.add("show"));return}if(s!==n){o&&(o.textContent="Passwords do not match.",o.classList.add("show"));return}o&&o.classList.remove("show"),l&&(l.disabled=!0),r&&(r.textContent="Registering…");try{await w.register({email:i,password:s,full_name:e,license_id:t,institution:a}),v("Account registered successfully! Please sign in.","success");const d=document.getElementById("login-email");d&&(d.value=i),S("view-login")}catch(d){o&&(o.textContent=d.message||"Registration failed",o.classList.add("show"))}finally{l&&(l.disabled=!1),r&&(r.textContent="Register Credentials")}},window.PathoApp.handleSendOTP=async()=>{const e=document.getElementById("forgot-email"),t=document.getElementById("forgot-error"),a=document.getElementById("forgot-success"),i=document.getElementById("btn-send-otp"),s=document.getElementById("send-otp-btn-text"),n=e?e.value.trim():"";if(!n){t&&(t.textContent="Please enter your registered email.",t.classList.add("show"));return}t&&t.classList.remove("show"),a&&(a.style.display="none"),i&&(i.disabled=!0),s&&(s.textContent="Sending OTP…");try{const o=await w.forgotPassword(n);window.PathoApp._otpEmail=n;const l=document.getElementById("forgot-step1"),r=document.getElementById("forgot-step2"),d=document.getElementById("otp-email-display");l&&(l.style.display="none"),r&&(r.style.display="block"),d&&(d.textContent=n),v(o.msg||"OTP sent! Please check your email for the 6-digit code.","success")}catch(o){t&&(t.textContent=o.message||"Failed to send OTP. Please try again.",t.classList.add("show"))}finally{i&&(i.disabled=!1),s&&(s.textContent="Send OTP Code")}},window.PathoApp.handleResendOTP=async()=>{const e=window.PathoApp._otpEmail||(document.getElementById("forgot-email")?document.getElementById("forgot-email").value.trim():""),t=document.getElementById("otp-verify-error"),a=document.getElementById("otp-verify-success");if(t&&t.classList.remove("show"),a&&(a.style.display="none"),!e){t&&(t.textContent="Email address missing. Please go back and enter your email.",t.classList.add("show"));return}try{const i=await w.forgotPassword(e);v("A new 6-digit OTP code has been sent to your email.","info"),a&&(a.textContent="New OTP code sent to your email address.",a.style.display="block")}catch(i){t&&(t.textContent=i.message||"Failed to resend OTP.",t.classList.add("show"))}},window.PathoApp.handleVerifyOTPReset=async()=>{const e=document.getElementById("otp-code-input"),t=document.getElementById("new-password-input"),a=document.getElementById("confirm-password-input"),i=document.getElementById("otp-verify-error"),s=document.getElementById("otp-verify-success"),n=document.getElementById("btn-verify-otp"),o=document.getElementById("verify-otp-btn-text"),l=e?e.value.trim():"",r=t?t.value:"",d=a?a.value:"",c=window.PathoApp._otpEmail||"";if(i&&i.classList.remove("show"),s&&(s.style.display="none"),!l||!/^\d{6}$/.test(l)){i&&(i.textContent="Please enter the 6-digit numeric OTP code.",i.classList.add("show"));return}if(!r||r.length<6){i&&(i.textContent="Password must be at least 6 characters.",i.classList.add("show"));return}if(r!==d){i&&(i.textContent="Passwords do not match.",i.classList.add("show"));return}if(!c){i&&(i.textContent="Session expired. Please go back and request a new OTP.",i.classList.add("show"));return}n&&(n.disabled=!0),o&&(o.textContent="Resetting…");try{const u=await w.verifyOTPReset(c,l,r);v("Password reset successful! Please sign in with your new password.","success"),s&&(s.textContent=u.msg||"Password reset successful! Redirecting to Sign In...",s.style.display="block"),window.PathoApp._otpEmail=null,setTimeout(()=>{e&&(e.value=""),t&&(t.value=""),a&&(a.value=""),window.PathoApp._resetForgotStep1(),S("view-login")},1500)}catch(u){i&&(i.textContent=u.message||"Invalid or expired OTP code.",i.classList.add("show"))}finally{n&&(n.disabled=!1),o&&(o.textContent="Reset Password")}},window.PathoApp._resetForgotStep1=()=>{const e=document.getElementById("forgot-step1"),t=document.getElementById("forgot-step2"),a=document.getElementById("forgot-error"),i=document.getElementById("otp-verify-error");e&&(e.style.display="block"),t&&(t.style.display="none"),a&&a.classList.remove("show"),i&&i.classList.remove("show"),window.PathoApp._otpEmail=null},window.PathoApp.handleLogout=async()=>{localStorage.removeItem("pathoai_logged_in");try{await w.logout()}catch(e){console.warn("Logout API error:",e)}finally{x({currentUser:null}),v("Logged out of PathoAI","info"),S("view-login")}}}function be(){const{currentUser:e,searchQuery:t}=m(),a=e?e.full_name||e.email.split("@")[0]:"Pathologist",i=new Date().getHours(),s=i<12?"Good Morning,":i<17?"Good Afternoon,":"Good Evening,",n=`https://ui-avatars.com/api/?name=${encodeURIComponent(a)}&background=38bdf8&color=0b172e&bold=true`,l=(localStorage.getItem("pathoai_theme")||"dark")==="dark";return`
    <header class="app-header">
      ...
      <div class="header-user">
        ...
        <!-- User Info -->
        <div class="user-info">
          <div class="user-greeting" id="hdr-greeting">${s}</div>
          <div class="user-name" id="hdr-name">${a}</div>
        </div>
        <img id="hdr-avatar" src="${n}" alt="Avatar" class="user-avatar" 
  ...
  `}function we(){window.PathoApp.handleSearch=t=>{x({searchQuery:t})},window.PathoApp.toggleSidebar=()=>{const t=document.querySelector(".sidebar");t&&t.classList.toggle("open")},window.PathoApp.toggleTheme=()=>{const a=(document.documentElement.getAttribute("data-theme")||"dark")==="dark"?"light":"dark";document.documentElement.setAttribute("data-theme",a),localStorage.setItem("pathoai_theme",a);const i=document.getElementById("theme-icon"),s=document.getElementById("theme-label");i&&(i.textContent=a==="dark"?"☀️":"🌙"),s&&(s.textContent=a==="dark"?"Day Mode":"Night Mode")};const e=localStorage.getItem("pathoai_theme")||"dark";document.documentElement.setAttribute("data-theme",e)}function Pe(){const{activePage:e,patients:t}=m(),a=t.filter(n=>n.status==="Pending").length,i=t.filter(n=>(n.risk_score||0)>=65).length;return`
    <aside class="sidebar">
      ...
      <nav class="nav-menu">
        ${[{id:"page-dashboard",icon:"fa-chart-pie",label:"Dashboard",badge:null},{id:"page-patients",icon:"fa-users",label:"Patients Directory",badge:t.length},{id:"page-upload",icon:"fa-cloud-upload-alt",label:"New Slide Upload",badge:a?`${a} new`:null},{id:"page-inspector",icon:"fa-microscope",label:"40x Slide Inspector",badge:i?`${i} alert`:null},{id:"page-profile",icon:"fa-user-md",label:"Doctor Profile",badge:null}].map(n=>`
          <div class="nav-item ${e===n.id?"active":""}" 
               onclick="window.PathoApp.setPage('${n.id}')">
            <i class="fas ${n.icon}"></i>
            <span style="flex:1;">${n.label}</span>
            ${n.badge?`<span class="badge ${n.badge.includes("alert")?"badge-high":"badge-pending"}">${n.badge}</span>`:""}
          </div>
        `).join("")}
      </nav>
  ...
  `}async function q(){try{const e=await w.getDashboard(),{stats:t,recent_patients:a}=e||{},{searchQuery:i,patients:s}=m();if(t){O("kpi-total",t.total_patients||0),O("kpi-analyzed",t.analyzed_patients||0),O("kpi-highrisk",t.high_risk_patients||0),O("kpi-pending",t.pending_patients||0);const l=t.total_patients||0,r=t.analyzed_patients||0,d=l>0?Math.round(r/l*100):0,c=document.getElementById("ring-circle"),u=document.getElementById("ring-pct");if(c){const p=Math.round(d/100*251);c.setAttribute("stroke-dasharray",`${p} 251`)}u&&(u.textContent=`${d}%`)}let n=a&&a.length>0?a:s;if(i&&n){const l=i.toLowerCase();n=n.filter(r=>(r.name||"").toLowerCase().includes(l)||(r.biopsy_site||"").toLowerCase().includes(l)||(r.diagnosis||"").toLowerCase().includes(l))}const o=document.getElementById("dash-patient-list");o&&(!n||n.length===0?o.innerHTML=K():o.innerHTML=n.slice(0,6).map(X).join(""))}catch(e){console.warn("Dashboard /dashboard endpoint failed:",e.message)}}function O(e,t){const a=document.getElementById(e);if(!a)return;const i=parseInt(a.textContent)||0,s=900,n=performance.now(),o=l=>{const r=Math.min((l-n)/s,1),d=1-Math.pow(1-r,3);a.textContent=Math.round(i+(t-i)*d),r<1&&requestAnimationFrame(o)};requestAnimationFrame(o)}function xe(){var p;const{patients:e,searchQuery:t}=m(),a=e.filter(g=>{if(!t)return!0;const h=t.toLowerCase();return g.name.toLowerCase().includes(h)||(g.biopsy_site||"").toLowerCase().includes(h)||(g.diagnosis||"").toLowerCase().includes(h)}),i=e.length,s=e.filter(g=>g.status!=="Pending").length,n=e.filter(g=>(g.risk_score||0)>=65).length,o=e.filter(g=>g.status==="Pending").length,l=[...a].slice(0,6),r=(g,h)=>g.map(P=>`
    <div style="flex:1; background:linear-gradient(180deg,var(${h}),color-mix(in srgb,var(${h}) 40%,transparent)); border-radius:3px 3px 0 0; height:${P}%; opacity:0.75; min-width:4px; transition:height 0.4s;"></div>
    ...
    <div id="page-dashboard" class="page-content active">
      ...
      ">
        ...
        <div style="position:relative;z-index:1;">
          ...
          <h1 style="font-size:1.75rem;font-weight:800;color:var(--text-main);margin:0 0 0.3rem;">${c<12?"Good morning":c<17?"Good afternoon":"Good evening"}, ${m().currentUser&&(m().currentUser.full_name||((p=m().currentUser.email)==null?void 0:p.split("@")[0]))||"Doctor"} <span style="font-size:1.5rem;">👋</span></h1>
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1.2rem;margin-bottom:2rem;" class="dash-kpi-row">
        ...
        <div class="dash-kpi-card" style="--kpi-accent:#10b981;--kpi-glow:rgba(16,185,129,0.18);cursor:pointer;"
          ...
          <div class="dash-kpi-inner">
            ...
            <div class="dash-kpi-value" id="kpi-analyzed">${s}</div>
        ...
        <div class="dash-kpi-card" style="--kpi-accent:#f43f5e;--kpi-glow:rgba(244,63,94,0.18);cursor:pointer;"
          ...
          <div class="dash-kpi-inner">
            ...
            <div class="dash-kpi-value" id="kpi-highrisk">${n}</div>
            <div class="dash-kpi-trend" style="color:#f43f5e;">
              <i class="fas fa-shield-alt"></i> Risk ≥ 65%
            </div>
            <div class="dash-kpi-spark" style="align-items:flex-end;">
              ${r([10,15,20,18,25,30,28,35,40,n>0?100:10],"--chart-5")}
            </div>
          </div>
          <div class="dash-kpi-glow"></div>
        </div>
          ...
          <div style="background:var(--card-bg);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:1.5rem;text-align:center;">
            ...
            <div class="ring-chart-wrap">
              <svg viewBox="0 0 100 100" width="130" height="130">
                ...
                <circle id="ring-circle" cx="50" cy="50" r="40" fill="none"
                  ...
                  stroke-dasharray="${s>0&&i>0?Math.round(s/i*251):0} 251"
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;margin-top:1.2rem;">
              <div style="background:rgba(56,189,248,0.08);border:1px solid rgba(56,189,248,0.18);border-radius:10px;padding:0.65rem;text-align:center;">
                <div style="font-size:1.1rem;font-weight:800;color:#38bdf8;">${s}</div>
  ...
  `}function ke(e){const t=e.filter(l=>(l.risk_score||0)>=65).length,a=e.filter(l=>(l.risk_score||0)>=40&&(l.risk_score||0)<65).length,i=e.filter(l=>(l.risk_score||0)>0&&(l.risk_score||0)<40).length,s=e.filter(l=>!l.risk_score||l.status==="Pending").length,n=Math.max(e.length,1),o=(l,r,d,c,u)=>`
    <div style="margin-bottom:0.9rem;cursor:pointer;" onclick="window.PathoApp.navigateToFilteredPatients('${u}')" title="Filter cases by ${l}">
      ...
      <div style="background:rgba(255,255,255,0.05);border-radius:4px;height:6px;overflow:hidden;">
        <div style="height:100%;width:${Math.round(r/n*100)}%;background:${d};border-radius:4px;transition:width 1s ease;"></div>
      </div>
    </div>
  `;return`
    ...
    ${o("Pending Analysis",s,"#64748b","fa-clock","pending")}
  `}function Ie(){const e=m(),t=!!e.currentUser,a=e.patients&&e.patients.length>0;return[{name:"FastAPI Backend",status:t?"online":"offline",color:t?"#10b981":"#f43f5e"},{name:"MongoDB Database",status:a?"online":"mock",color:a?"#10b981":"#f59e0b"},{name:"MIL AI Engine",status:"ready",color:"#38bdf8"},{name:"Image Processor",status:"ready",color:"#38bdf8"}].map(s=>`
  ...
  `).join("")}function X(e){const t=e.risk_score||0,a=t>=65?"#f43f5e":t>=40?"#f59e0b":t>0?"#10b981":"#64748b",i=t>=65?"High Risk":t>=40?"Moderate":t>0?"Low Risk":"Pending",s=t>=65?"rgba(244,63,94,0.12)":t>=40?"rgba(245,158,11,0.12)":t>0?"rgba(16,185,129,0.12)":"rgba(100,116,139,0.12)",n=e.id||e._id||e.patient_uid||"",o=(e.name||"?").split(" ").map(d=>d[0]).slice(0,2).join("").toUpperCase(),l=["#38bdf8","#8b5cf6","#10b981","#f59e0b","#f43f5e"],r=l[o.charCodeAt(0)%l.length];return`
    <div class="patient-card" onclick="window.PathoApp.openPatientDetails('${n}')" style="position:relative;overflow:hidden;">
      <!-- Subtle glow accent -->
  ...
  `}function Se(){const{patients:e,searchQuery:t,riskFilter:a}=m();let i=e.filter(s=>{if(a==="high"&&(s.risk_score||0)<65||a==="moderate"&&((s.risk_score||0)<40||(s.risk_score||0)>=65)||a==="low"&&(s.risk_score||0)>=40||a==="pending"&&s.status!=="Pending"||a==="analyzed"&&s.status==="Pending")return!1;if(!t)return!0;const n=t.toLowerCase();return(s.name||"").toLowerCase().includes(n)||(s.patient_uid||"").toLowerCase().includes(n)||(s.biopsy_site||"").toLowerCase().includes(n)||(s.tissue_type||"").toLowerCase().includes(n)||(s.diagnosis||"").toLowerCase().includes(n)});return`
    <div id="page-patients" class="page-content">
      ...
      <div class="patient-grid" id="full-patient-list">
        ${i.length>0?i.map(s=>Ae(s)).join(""):_e()}
      </div>
  ...
  `}function Ae(e){const t=e.risk_score>=65?"badge-high":e.risk_score>=40?"badge-mod":e.status==="Pending"?"badge-pending":"badge-low";return`
  ...
  `}function Ce(){window.PathoApp.setRiskFilter=e=>{x({riskFilter:e})},window.PathoApp.openAddPatientModal=()=>{const e=document.getElementById("modal-add-patient");e&&e.classList.add("active")},window.PathoApp.closeAddPatientModal=()=>{const e=document.getElementById("modal-add-patient");e&&e.classList.remove("active")},window.PathoApp.handleAddPatient=async()=>{const e=document.getElementById("p-name").value.trim(),t=document.getElementById("p-age").value,a=document.getElementById("p-gender").value,i=document.getElementById("p-site").value.trim(),s=document.getElementById("p-notes").value.trim();if(!e||!t||!i){v("Please fill in name, age, and biopsy site.","error");return}const n=document.querySelector('#form-add-patient button[type="submit"]');n&&(n.disabled=!0,n.textContent="Saving…");try{const o=await w.createPatient({name:e,age:parseInt(t),gender:a,biopsy_site:i,notes:s}),l=await w.getPatients();x({patients:l});const r=document.getElementById("full-patient-list");r&&l.length>0&&(r.innerHTML=l.map(d=>{const c=(d.risk_score||0)>=65?"badge-high":(d.risk_score||0)>=40?"badge-mod":d.status==="Pending"?"badge-pending":"badge-low";return`<div class="patient-card" onclick="window.PathoApp.openPatientDetails('${d.id||d.patient_uid||""}')">
          ...
          </div>`}).join("")),v(`Patient case for ${e} registered & saved to MongoDB!`,"success"),window.PathoApp.closeAddPatientModal(),["p-name","p-age","p-site","p-notes"].forEach(d=>{const c=document.getElementById(d);c&&(c.value="")})}catch(o){v(o.message||"Failed to create patient record","error")}finally{n&&(n.disabled=!1,n.textContent="Register Patient Case")}},window.PathoApp.openPatientDetails=e=>{const t=m().patients.find(a=>String(a.id)===String(e)||String(a._id)===String(e)||String(a.patient_uid)===String(e));t?(M(t),E("page-inspector")):v("Patient record not found.","error")}}function ee(e){if(!e)return"/pathology_slide_sample.png";if(e.startsWith("data:")||e.startsWith("http"))return e;if(e.startsWith("/uploads/")||e.startsWith("uploads/")){const t=e.startsWith("/")?e:`/${e}`;return`${j()}${t}`}return e.includes("pathology_slide_sample.png")?"/pathology_slide_sample.png":e}let T=null;function te(){const{activePatient:e,inspector:t,patients:a}=m(),i=e||a[0]||null;if(!i)return`
    ...
    <div id="page-inspector" class="page-content">
      ...
      <div style="background:var(--card-bg); border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:1.2rem 1.5rem; margin-bottom:1.5rem; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:1rem;">
        <div style="display:flex; align-items:center; gap:1.2rem;">
          <img src="${ee(i.localImageUrl||i.image_url)||"/pathology_slide_sample.png"}" 
        ...
        <div style="display:flex; gap:1.5rem; align-items:center;">
          ...
          <div style="display:flex; background:var(--input-bg); border:1px solid var(--border-color); border-radius:var(--radius-sm); padding:3px;">
            <button class="btn-secondary ${s==="original"?"active":""}" style="padding:0.4rem 0.8rem; font-size:0.75rem;" onclick="window.PathoApp.setInspectorMode('original')">
              Original Slide
            </button>
            <button class="btn-secondary ${s==="mask"?"active":""}" style="padding:0.4rem 0.8rem; font-size:0.75rem;" onclick="window.PathoApp.setInspectorMode('mask')">
              Segmentation Mask
            </button>
            <button class="btn-secondary ${s==="overlay"?"active":""}" style="padding:0.4rem 0.8rem; font-size:0.75rem;" onclick="window.PathoApp.setInspectorMode('overlay')">
              Overlaid Mask
            </button>
          </div>
  ...
  `}function ie(){const e=document.getElementById("slide-canvas");if(!e)return;const t=e.getContext("2d"),a=e.parentElement.getBoundingClientRect();e.width=a.width||800,e.height=a.height||500;const{activePatient:i,patients:s}=m(),n=i||s[0];if(!n)return;const o=new Image,l=ee(n.localImageUrl||n.image_url)||"/pathology_slide_sample.png";o.src=l,o.onload=()=>{T=o,r()},o.onerror=()=>{o.src!==window.location.origin+"/pathology_slide_sample.png"&&o.src!=="/pathology_slide_sample.png"&&(o.src="/pathology_slide_sample.png")};function r(){if(!t||!T)return;const{inspector:p}=m(),g=p.mode||"overlay";t.clearRect(0,0,e.width,e.height),t.save();const h=e.width/2+(p.panX||0),P=e.height/2+(p.panY||0),B=p.zoom||1,I=T.width*B*.45,_=T.height*B*.45;g==="original"||g==="overlay"?t.drawImage(T,h-I/2,P-_/2,I,_):g==="mask"&&(t.fillStyle="#060d1d",t.fillRect(h-I/2,P-_/2,I,_)),(g==="mask"||g==="overlay")&&p.showHeatmap!==!1&&(t.fillStyle="rgba(244, 63, 94, 0.35)",t.beginPath(),t.arc(h,P-20,I*.22,0,Math.PI*2),t.fill(),t.fillStyle="rgba(245, 158, 11, 0.28)",t.beginPath(),t.arc(h+I*.2,P+30,I*.18,0,Math.PI*2),t.fill()),p.showBoundingBoxes!==!1&&(t.strokeStyle="#38bdf8",t.lineWidth=2,t.strokeRect(h-80,P-60,160,120),t.fillStyle="#38bdf8",t.font='600 12px "Plus Jakarta Sans", sans-serif',t.fillText(`ROI Tile #42 · ${n.diagnosis||"Oral Dysplasia"}`,h-78,P-68)),t.restore()}let d=!1,c,u;e.onmousedown=p=>{d=!0,c=p.clientX,u=p.clientY},window.onmousemove=p=>{if(!d)return;const g=p.clientX-c,h=p.clientY-u;c=p.clientX,u=p.clientY;const{inspector:P}=m();P.panX=(P.panX||0)+g,P.panY=(P.panY||0)+h,r()},window.onmouseup=()=>{d=!1},window.PathoApp.setInspectorMode=p=>{const{inspector:g}=m();g.mode=p,x({inspector:g}),r()},window.PathoApp.zoomCanvas=p=>{const{inspector:g}=m();g.zoom=Math.max(.5,Math.min(4,(g.zoom||1)+p));const h=document.getElementById("zoom-level-text");h&&(h.textContent=`${Math.round(g.zoom*10)}x`),r()},window.PathoApp.toggleHeatmap=()=>{const{inspector:p}=m();p.showHeatmap=!p.showHeatmap,r()},window.PathoApp.toggleBoundingBoxes=()=>{const{inspector:p}=m();p.showBoundingBoxes=!p.showBoundingBoxes,r()},window.PathoApp.resetCanvas=()=>{const{inspector:p}=m();p.zoom=1,p.panX=0,p.panY=0;const g=document.getElementById("zoom-level-text");g&&(g.textContent="10x"),r()}}let D=null,C=null;function Ee(){const{patients:e}=m();return`
  ...
  `}function $e(){setTimeout(()=>{const e=document.getElementById("drop-zone");e&&(e.addEventListener("dragover",t=>{t.preventDefault(),e.style.borderColor="var(--primary)",e.style.background="rgba(56,189,248,0.12)"}),e.addEventListener("dragleave",()=>{e.style.borderColor="var(--primary-light)",e.style.background="rgba(6,13,29,0.5)"}),e.addEventListener("drop",t=>{t.preventDefault(),e.style.borderColor="var(--primary-light)",e.style.background="rgba(6,13,29,0.5)",t.dataTransfer.files&&t.dataTransfer.files[0]&&window.PathoApp.handleFileSelected(t.dataTransfer.files[0])}))},100),window.PathoApp.handleFileSelected=e=>{if(!e)return;D=e;const t=new FileReader;t.onload=a=>{C=a.target.result;const i=document.getElementById("preview-img-src"),s=document.getElementById("preview-filename"),n=document.getElementById("preview-filesize"),o=document.getElementById("file-preview-card");i&&(i.src=C),s&&(s.textContent=e.name),n&&(n.textContent=`${(e.size/(1024*1024)).toFixed(2)} MB · Uploaded Image Ready for Scan`),o&&(o.style.display="block")},t.readAsDataURL(e)},window.PathoApp.clearSelectedFile=()=>{D=null,C=null;const e=document.getElementById("file-preview-card"),t=document.getElementById("slide-file-input");e&&(e.style.display="none"),t&&(t.value="")},window.PathoApp.runPipelineAnalysis=async()=>{var s;const e=document.getElementById("pipeline-stepper"),t=document.getElementById("btn-run-pipeline"),a=((s=document.getElementById("select-patient-case"))==null?void 0:s.value)||null;if(!a){v("Please select a patient case before running analysis.","error");return}if(!D){v("Please upload a pathology slide image first.","error");return}e&&(e.style.display="block"),t&&(t.disabled=!0,t.innerHTML='<i class="fas fa-spinner fa-spin"></i> Running Deep MIL Analysis…');const i=(n,o,l=!1)=>setTimeout(()=>{const r=document.getElementById(n);if(!r)return;r.style.opacity="1";const d=r.querySelector("i");d&&(l?(d.className="fas fa-check-circle",d.style.color="var(--success)"):(d.className="fas fa-spinner fa-spin",d.style.color="var(--primary-light)"))},o);i("step-2",700),i("step-3",1400),i("step-4",2100),i("step-4",2800,!0);try{await new Promise(d=>setTimeout(d,2900));const{currentUser:n}=m(),o=await w.analyzeSlide(D,a,(n==null?void 0:n.id)||null);v("✓ Deep MIL Analysis complete — results updated!","success");const l=await w.getPatients();let r=l.find(d=>String(d.id)===String(a)||String(d.patient_uid)===String(a));o?(r||(r={id:a}),(o.prediction||o.diagnosis)&&(r.diagnosis=o.prediction||o.diagnosis),(o.risk_score!==void 0||o.score!==void 0)&&(r.risk_score=o.risk_score??o.score),o.confidence!==void 0&&(r.confidence=o.confidence),o.notes&&(r.notes=o.notes),(o.heatmap_url||o.file_url)&&(r.image_url=o.heatmap_url||o.file_url),C&&(r.localImageUrl=C)):C&&r&&(r.image_url=C),x({patients:l}),r?M(r):l.length>0&&M(l[0]),setTimeout(()=>E("page-inspector"),500)}catch(n){v(`Analysis failed: ${n.message}`,"error"),console.error("[AnalysisPipeline] Error:",n)}finally{t&&(t.disabled=!1,t.innerHTML='<i class="fas fa-play"></i> Execute Deep MIL Analysis Pipeline')}}}function Be(e){if(!e)return"/pathology_slide_sample.png";if(e.startsWith("data:")||e.startsWith("http"))return e;if(e.startsWith("/uploads/")||e.startsWith("uploads/")){const t=e.startsWith("/")?e:`/${e}`;return`${j()}${t}`}return e.includes("pathology_slide_sample.png")?"/pathology_slide_sample.png":e}function ae(){const{activePatient:e,currentUser:t,patients:a}=m(),i=e||(a&&a.length>0?a[0]:null);if(!i)return`
    ...
    `;const s=t?t.full_name||t.email:"Consultant Pathologist",n=t&&t.institution||"PathoAI Clinical Center",o=t?t.license_id||"License Not Specified":"N/A";return`
    <div id="page-report" class="page-content">
      <div class="page-header">
        <div>
          <h1 class="page-title">Diagnostic Pathology Report</h1>
      ...
      <div style="background:#ffffff; color:#0f172a; border-radius:var(--radius-lg); padding:3rem; box-shadow:var(--shadow-card); max-width:860px; margin:0 auto;" id="printable-report">
        ...
        <div style="display:flex; justify-content:space-between; align-items:flex-start; border-bottom:2px solid #e2e8f0; padding-bottom:1.5rem; margin-bottom:1.8rem;">
          <div>
            ...
            <div style="font-size:0.85rem; color:#64748b; margin-top:0.2rem;">${n} · Oral Pathology Department</div>
          </div>
          <div style="text-align:right;">
        ...
        <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; padding:1.2rem; margin-bottom:1.8rem;">
          <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:1rem; font-size:0.9rem;">
            ...
            <div><strong style="color:#475569;">Attending Doctor:</strong> <span style="font-weight:700; color:#0f172a;">${s}</span></div>
  ...
  `}function ze(){const{currentUser:e}=m(),t=e?e.full_name||e.email.split("@")[0]:"Clinical Pathologist",a=e?e.email:"No user logged in",i=e&&e.license_id||"",s=e&&e.institution||"",n=e&&e.role?`Consultant ${e.role.charAt(0).toUpperCase()+e.role.slice(1)}`:"Consultant Pathologist";return`
    <div id="page-profile" class="page-content">
      ...
      <div style="display:grid; grid-template-columns: 300px 1fr; gap:1.8rem; align-items:start;">
        ...
        <div style="background:var(--card-bg); border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:2rem; text-align:center;">
          ...
          <div style="font-size:0.85rem; color:var(--primary-light); font-weight:600; margin-top:0.2rem;" id="prof-role">${n}</div>
          ...
          <div style="margin-top:1.5rem; padding-top:1.2rem; border-top:1px solid var(--border-color); font-size:0.82rem; color:var(--text-muted);">
            <div id="prof-institution" style="font-weight:600; color:var(--text-main);">${s||"PathoAI Medical Center"}</div>
        ...
        <div style="display:flex; flex-direction:column; gap:1.5rem;">
          ...
          <div style="background:var(--card-bg); border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:2rem;">
            ...
            <form onsubmit="event.preventDefault(); window.PathoApp.saveProfileSettings();">
              ...
              <div class="input-wrap">
                <i class="fas fa-hospital"></i>
                <input type="text" id="edit-prof-inst" value="${s}" placeholder="Hospital / Medical Institution">
              </div>
  ...
  `}function Le(){window.PathoApp._togglePwd=(e,t)=>{const a=document.getElementById(e);if(!a)return;const i=a.type==="password";a.type=i?"text":"password";const s=t.querySelector("i");s&&(s.className=i?"fas fa-eye-slash":"fas fa-eye")},window.PathoApp._updatePwdStrength=(e,t)=>{const a=document.getElementById(t);if(!a)return;let i=0;e.length>=8&&i++,e.length>=12&&i++,/[A-Z]/.test(e)&&i++,/[0-9]/.test(e)&&i++,/[^A-Za-z0-9]/.test(e)&&i++;const s=["#ef4444","#f97316","#eab308","#22c55e","#16a34a"],n=["20%","40%","60%","80%","100%"];a.style.background=s[i-1]||"#374151",a.style.width=i>0?n[i-1]:"0"},window.PathoApp.saveProfileSettings=async()=>{const e=document.getElementById("edit-prof-name"),t=document.getElementById("edit-prof-lic"),a=document.getElementById("edit-prof-inst"),i=document.getElementById("profile-save-error"),s=document.getElementById("profile-save-success"),n=document.getElementById("btn-save-profile"),o=document.getElementById("save-profile-btn-text");i&&(i.style.display="none",i.textContent=""),s&&(s.style.display="none",s.textContent="");const l=e?e.value.trim():"",r=t?t.value.trim():"",d=a?a.value.trim():"";if(!l||l.length<2){i&&(i.textContent="Full name must be at least 2 characters.",i.style.display="block");return}n&&(n.disabled=!0),o&&(o.textContent="Saving…");try{const c=await w.updateProfile({full_name:l,license_id:r,institution:d}),{currentUser:u}=m(),p={...u,full_name:c.full_name||l,institution:c.institution||d,license_id:c.license_id||r};x({currentUser:p});const g=document.getElementById("prof-name"),h=document.getElementById("prof-institution"),P=document.getElementById("prof-id"),B=document.getElementById("prof-avatar"),I=document.getElementById("hdr-name"),_=document.getElementById("hdr-avatar");g&&(g.textContent=c.full_name||l),h&&(h.textContent=c.institution||d||"PathoAI Medical Center"),P&&(P.textContent=`License: ${c.license_id||r||"Not specified"}`);const H=`https://ui-avatars.com/api/?name=${encodeURIComponent(c.full_name||l)}&background=38bdf8&color=0b172e&bold=true&size=128`;B&&(B.src=H),I&&(I.textContent=c.full_name||l),_&&(_.src=H),s&&(s.textContent="✓ Profile saved to MongoDB successfully!",s.style.display="block"),v("Profile credentials saved to MongoDB!","success")}catch(c){const u=c.message||"Failed to save profile. Please try again.";i&&(i.textContent=u,i.style.display="block"),v(u,"error")}finally{n&&(n.disabled=!1),o&&(o.textContent="Save Profile")}},window.PathoApp.changePassword=async()=>{const e=document.getElementById("chg-current-pass"),t=document.getElementById("chg-new-pass"),a=document.getElementById("chg-confirm-pass"),i=document.getElementById("chgpwd-error"),s=document.getElementById("chgpwd-success"),n=document.getElementById("btn-change-pwd"),o=document.getElementById("change-pwd-btn-text");i&&(i.style.display="none",i.textContent=""),s&&(s.style.display="none",s.textContent="");const l=e?e.value:"",r=t?t.value:"",d=a?a.value:"";if(!l){i&&(i.textContent="Please enter your current password.",i.style.display="block");return}if(!r||r.length<8){i&&(i.textContent="New password must be at least 8 characters.",i.style.display="block");return}if(r!==d){i&&(i.textContent="New passwords do not match.",i.style.display="block");return}n&&(n.disabled=!0),o&&(o.textContent="Changing…");try{const c=await w.changePassword(l,r);s&&(s.textContent=`✓ ${c.msg||"Password changed successfully."}`,s.style.display="block"),v("Password changed successfully!","success"),e&&(e.value=""),t&&(t.value=""),a&&(a.value="");const u=document.getElementById("chg-strength");u&&(u.style.background="",u.style.width="0")}catch(c){const u=c.message||"Failed to change password. Please check your current password.";i&&(i.textContent=u,i.style.display="block")}finally{n&&(n.disabled=!1),o&&(o.textContent="Change Password")}},window.PathoApp.exportDatabaseBackup=async()=>{try{await ue(),v("Database JSON backup downloaded!","success")}catch(e){v("Database export failed: "+e.message,"error")}}}window.PathoApp=window.PathoApp||{};window.PathoApp.setPage=e=>E(e);window.PathoApp.setView=e=>S(e);window.PathoApp.showView=e=>S(e);window.PathoApp.switchPage=e=>E(e);window.PathoApp.showToast=(e,t)=>v(e,t);window.PathoApp.getState=()=>m();window.PathoApp.setState=e=>x(e);window.PathoApp.refreshDashboard=q;window.PathoApp.openPatientDetails=e=>{const{patients:t}=m(),a=t.find(i=>String(i.id)===String(e)||String(i._id)===String(e)||String(i.patient_uid)===String(e));a?(M(a),E("page-inspector")):v("Patient record not found.","error")};window.PathoApp.navigateToFilteredPatients=e=>{x({riskFilter:e}),E("page-patients")};window.showView=e=>S(e);window.switchPage=e=>E(e);window.enterApp=()=>S("view-app");window.toast=(e,t)=>v(e,t);window.qs=e=>document.querySelector(e);document.addEventListener("DOMContentLoaded",async()=>{try{await k()}catch(e){console.warn("IndexedDB Init Warning:",e)}Te(),ye(),we(),Ce(),$e(),Le(),se(Oe),setTimeout(async()=>{try{const e=await w.getCurrentUser();x({currentUser:e}),localStorage.setItem("pathoai_logged_in","true");try{const t=await w.getPatients();x({patients:t})}catch(t){console.warn("Patient fetch warning:",t.message)}S("view-app"),setTimeout(q,200)}catch{localStorage.removeItem("pathoai_logged_in"),S("view-login")}},1200)});function Te(){const e=document.getElementById("app-shell");if(!e)return;e.innerHTML=`
    <!-- Ambient Backdrop -->
  ...
  `,document.querySelectorAll(".view").forEach(i=>{i.classList.remove("active"),i.style.display="none"});const t=document.getElementById("view-splash");t&&(t.classList.add("active"),t.style.display="flex"),document.querySelectorAll(".page-content").forEach(i=>{i.classList.remove("active"),i.style.display="none"});const a=document.getElementById("page-dashboard");a&&(a.classList.add("active"),a.style.display="flex"),ie()}let G=null,J=null,W=null,Y=null,Q=null;function Oe(e){var l;const t=e.activePatient?e.activePatient.id||e.activePatient.patient_uid:null,a=t!==W;a&&(W=t);const i=e.searchQuery!==Y;i&&(Y=e.searchQuery);const s=e.riskFilter!==Q;s&&(Q=e.riskFilter),e.activeView!==G&&(G=e.activeView,document.querySelectorAll(".view").forEach(r=>{const d=r.id===e.activeView;r.classList.toggle("active",d),r.style.display=d?"flex":"none"}));const n=e.activePage!==J;if(n&&(J=e.activePage,document.querySelectorAll(".page-content").forEach(r=>{const d=r.id===e.activePage;r.classList.toggle("active",d),r.style.display=d?"flex":"none"})),(n||a||i||s)&&(e.activePage==="page-dashboard"&&q(),e.activePage==="page-patients"&&De(),e.activePage==="page-inspector"&&Me(),e.activePage==="page-upload"&&Re(),e.activePage==="page-report"&&Ne(),e.activePage==="page-profile"&&Fe()),document.querySelectorAll(".nav-item").forEach(r=>{var c,u;const d=(u=(c=r.getAttribute("onclick"))==null?void 0:c.match(/'([^']+)'/))==null?void 0:u[1];r.classList.toggle("active",d===e.activePage)}),e.activePage==="page-dashboard"){const{patients:r}=e,d=document.getElementById("kpi-total"),c=document.getElementById("kpi-analyzed"),u=document.getElementById("kpi-highrisk");d&&(d.textContent=r.length),c&&(c.textContent=r.filter(p=>p.status!=="Pending").length),u&&(u.textContent=r.filter(p=>(p.risk_score||0)>=65).length)}const o=document.getElementById("toast");if(o&&(e.toast.visible?(o.textContent=e.toast.message,o.className=`toast show ${e.toast.type}`):o.classList.remove("show")),e.currentUser){const r=document.getElementById("hdr-name"),d=document.getElementById("hdr-avatar");if(r&&(r.textContent=e.currentUser.full_name||((l=e.currentUser.email)==null?void 0:l.split("@")[0])||""),d){const c=encodeURIComponent(e.currentUser.full_name||e.currentUser.email||"U");d.src=`https://ui-avatars.com/api/?name=${c}&background=38bdf8&color=0b172e&bold=true`}}}async function De(){try{const{searchQuery:e,riskFilter:t}=m();let a;try{a=await w.getPatients(),x({patients:a})}catch(n){console.warn("Patient fetch failed, using cached state:",n.message),a=m().patients}const i=a.filter(n=>{if(t==="high"&&(n.risk_score||0)<65||t==="moderate"&&((n.risk_score||0)<40||(n.risk_score||0)>=65)||t==="low"&&(n.risk_score||0)>=40||t==="pending"&&n.status!=="Pending"||t==="analyzed"&&n.status==="Pending")return!1;if(!e)return!0;const o=e.toLowerCase();return(n.name||"").toLowerCase().includes(o)||(n.patient_uid||"").toLowerCase().includes(o)||(n.biopsy_site||"").toLowerCase().includes(o)||(n.tissue_type||"").toLowerCase().includes(o)||(n.diagnosis||"").toLowerCase().includes(o)});document.querySelectorAll("#page-patients .btn-secondary").forEach(n=>{const o=n.getAttribute("onclick")||"";if(o.includes("setRiskFilter")){const l=o.includes(`'${t}'`);n.classList.toggle("active",l),o.includes("'all'")&&(n.textContent=`All Cases (${a.length})`,l&&n.classList.add("active"))}});const s=document.getElementById("full-patient-list");s&&(i.length===0?s.innerHTML=je():(s.innerHTML=i.map(n=>Ue(n)).join(""),s.querySelectorAll(".patient-card").forEach(n=>{n.addEventListener("click",()=>{const o=n.dataset.pid;window.PathoApp.openPatientDetails(o)})})))}catch(e){console.warn("Patient list refresh failed:",e.message)}}function Re(){const e=document.getElementById("select-patient-case");if(!e)return;const{patients:t}=m();e.innerHTML='<option value="">-- Create New Case or Select Patient --</option>'+t.map(a=>`<option value="${a.id||a._id||a.patient_uid}">${a.name} (${a.biopsy_site||"Oral Cavity"})</option>`).join("")}function Me(){const e=document.getElementById("page-inspector");if(!e)return;const t=e.classList.contains("active"),a=document.createElement("div");a.innerHTML=te();const i=a.firstElementChild;t&&i.classList.add("active"),e.replaceWith(i),setTimeout(ie,50)}function Ne(){const e=document.getElementById("page-report");if(!e)return;const t=e.classList.contains("active"),a=document.createElement("div");a.innerHTML=ae();const i=a.firstElementChild;t&&i.classList.add("active"),e.replaceWith(i)}function Fe(){const{currentUser:e}=m();if(!e)return;const t=document.getElementById("prof-name"),a=document.getElementById("prof-email"),i=document.getElementById("prof-institution"),s=document.getElementById("prof-id");t&&(t.textContent=e.full_name||""),a&&(a.textContent=e.email||""),i&&(i.textContent=e.institution||"PathoAI Medical Center"),s&&(s.textContent=`License: ${e.license_id||"Not specified"}`)}function Ue(e){const t=(e.risk_score||0)>=65?"badge-high":(e.risk_score||0)>=40?"badge-mod":e.status==="Pending"?"badge-pending":"badge-low",a=e.id||e._id||e.patient_uid||"";return`
...
//# sourceMappingURL=index-CE2l3Wx0.js.map
