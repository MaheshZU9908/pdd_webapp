(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const s of n)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function a(n){const s={};return n.integrity&&(s.integrity=n.integrity),n.referrerPolicy&&(s.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?s.credentials="include":n.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(n){if(n.ep)return;n.ep=!0;const s=a(n);fetch(n.href,s)}})();const M=new Set,I={activeView:"view-splash",activePage:"page-dashboard",currentUser:null,patients:[],activePatient:null,searchQuery:"",riskFilter:"all",inspector:{zoom:1,panX:0,panY:0,showHeatmap:!0,showBoundingBoxes:!0,brightness:100,contrast:100},toast:{message:"",type:"info",visible:!1}},u=()=>I,w=e=>{Object.assign(I,e),C()},ie=e=>(M.add(e),()=>M.delete(e));function C(){M.forEach(e=>e(I))}const v=(e,t="info")=>{I.toast={message:e,type:t,visible:!0},C(),setTimeout(()=>{I.toast.visible=!1,C()},3500)},k=e=>{I.activeView=e,C()},_=e=>{I.activePage=e,C()},R=e=>{I.activePatient=e,C()},ae="PathoAI_Frontend_DB",Y=1;let $=null;const se=[{id:1,patient_uid:"PT-2026-001",name:"Rajesh Sharma",age:54,gender:"Male",biopsy_site:"Lateral Tongue",tissue_type:"Epithelial Tissue",status:"Analyzed",risk_score:88,diagnosis:"Oral Squamous Cell Carcinoma (OSCC) - Stage II",date:"2026-08-15",created_at:"2026-08-15T09:30:00Z",notes:"Well-differentiated invasive squamous cell carcinoma with keratin pearls."},{id:2,patient_uid:"PT-2026-002",name:"Priya Sundaram",age:42,gender:"Female",biopsy_site:"Buccal Mucosa",tissue_type:"Epithelial Lesion",status:"Analyzed",risk_score:52,diagnosis:"Moderate Epithelial Dysplasia (Premalignant)",date:"2026-08-16",created_at:"2026-08-16T11:15:00Z",notes:"Hyperchromatism and basal layer hyperplasia observed."},{id:3,patient_uid:"PT-2026-003",name:"Anand Verma",age:38,gender:"Male",biopsy_site:"Hard Palate",tissue_type:"Mucosal Biopsy",status:"Analyzed",risk_score:18,diagnosis:"Benign Hyperkeratosis (Non-Malignant)",date:"2026-08-17",created_at:"2026-08-17T14:20:00Z",notes:"No architectural atypia. Regular stratified squamous epithelium."},{id:4,patient_uid:"PT-2026-004",name:"Sunita Reddy",age:49,gender:"Female",biopsy_site:"Gingiva",tissue_type:"Gingival Biopsy",status:"Pending",risk_score:0,diagnosis:"Pending Analysis",date:"2026-08-18",created_at:"2026-08-18T08:45:00Z",notes:"Awaiting high-power 40x slide analysis."}],ne=[{key:"ai_threshold",value:.65},{key:"gradcam_layer",value:"features.7.2"},{key:"theme",value:"dark"}];async function P(){return $||new Promise((e,t)=>{const a=indexedDB.open(ae,Y);a.onupgradeneeded=i=>{const n=i.target.result;if(!n.objectStoreNames.contains("patients")){const s=n.createObjectStore("patients",{keyPath:"id",autoIncrement:!0});s.createIndex("name","name",{unique:!1}),s.createIndex("risk_score","risk_score",{unique:!1}),s.createIndex("status","status",{unique:!1}),s.createIndex("biopsy_site","biopsy_site",{unique:!1})}n.objectStoreNames.contains("slides")||n.createObjectStore("slides",{keyPath:"id"}).createIndex("patient_id","patient_id",{unique:!1}),n.objectStoreNames.contains("settings")||n.createObjectStore("settings",{keyPath:"key"}),n.objectStoreNames.contains("session")||n.createObjectStore("session",{keyPath:"key"})},a.onsuccess=async i=>{$=i.target.result,await oe($),e($)},a.onerror=i=>{console.error("IndexedDB open error:",i.target.error),t(i.target.error)}})}async function oe(e){if(await re(e,"patients")===0){const a=e.transaction(["patients","settings"],"readwrite"),i=a.objectStore("patients"),n=a.objectStore("settings");return se.forEach(s=>i.add(s)),ne.forEach(s=>n.add(s)),new Promise(s=>{a.oncomplete=()=>{console.log("IndexedDB seeded with initial clinical biopsy records."),s()}})}}function re(e,t){return new Promise(a=>{const n=e.transaction(t,"readonly").objectStore(t).count();n.onsuccess=()=>a(n.result),n.onerror=()=>a(0)})}async function N(){const e=await P();return new Promise((t,a)=>{const s=e.transaction("patients","readonly").objectStore("patients").getAll();s.onsuccess=()=>t(s.result||[]),s.onerror=()=>a(s.error)})}async function le(e){const t=await P();return new Promise((a,i)=>{const o=t.transaction("patients","readonly").objectStore("patients").get(Number(e));o.onsuccess=()=>a(o.result),o.onerror=()=>i(o.error)})}async function B(e){const t=await P();return new Promise((a,i)=>{const s=t.transaction("patients","readwrite").objectStore("patients");e.date||(e.date=new Date().toISOString().split("T")[0]);const o=e.id?s.put(e):s.add(e);o.onsuccess=l=>{const r=e.id||l.target.result;a({...e,id:r})},o.onerror=()=>i(o.error)})}async function de(e){const t=await P();return new Promise((a,i)=>{const o=t.transaction("patients","readwrite").objectStore("patients").delete(Number(e));o.onsuccess=()=>a(!0),o.onerror=()=>i(o.error)})}async function O(e){const t=await P();return new Promise(a=>{const s=t.transaction("settings","readonly").objectStore("settings").get(e);s.onsuccess=()=>a(s.result?s.result.value:null),s.onerror=()=>a(null)})}async function ce(e,t){const a=await P();return new Promise((i,n)=>{const l=a.transaction("settings","readwrite").objectStore("settings").put({key:e,value:t});l.onsuccess=()=>i(!0),l.onerror=()=>n(l.error)})}async function pe(){const e=await N(),t=await O("doctor_profile"),a={app:"PathoAI Clinical Suite",version:Y,export_date:new Date().toISOString(),patients:e,doctor_profile:t},i=JSON.stringify(a,null,2),n=new Blob([i],{type:"application/json"}),s=URL.createObjectURL(n),o=document.createElement("a");o.href=s,o.download=`PathoAI_Database_Backup_${new Date().toISOString().split("T")[0]}.json`,o.click(),URL.revokeObjectURL(s)}const U=()=>{const e=window.PATHOAI_API_BASE_URL;return e&&e.startsWith("http")?e:""},L=()=>window.location.hostname.includes("github.io")||window.location.protocol==="file:";async function f(e,t={}){const a=`${U()}${e}`,n=t.body instanceof FormData?t.headers||{}:{"Content-Type":"application/json",...t.headers||{}},s=await fetch(a,{credentials:"include",headers:n,...t});if(!s.ok){let o=`HTTP ${s.status}`;try{const r=await s.json();o=r.detail||r.message||o}catch{}const l=new Error(o);throw l.status=s.status,l}return s.status===204?null:await s.json()}const F=()=>{try{return JSON.parse(localStorage.getItem("pathoai_users")||"[]")}catch{return[]}},ge=e=>{const t=F(),a=t.findIndex(i=>i.email.toLowerCase()===e.email.toLowerCase());a>=0?t[a]={...t[a],...e}:t.push(e),localStorage.setItem("pathoai_users",JSON.stringify(t))},y={getCurrentUser:async()=>{try{return await f("/auth/me")}catch(e){const t=localStorage.getItem("pathoai_active_user");if(t)return JSON.parse(t);throw e}},login:async(e,t)=>{const a=new URLSearchParams({username:e,password:t});try{const i=await f("/auth/login",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:a});return i&&i.user&&localStorage.setItem("pathoai_active_user",JSON.stringify(i.user)),i}catch(i){const s=F().find(o=>o.email.toLowerCase()===e.toLowerCase());if(s){if(s.password&&s.password!==t)throw new Error("Invalid email or password.");const o={email:s.email,full_name:s.full_name||e.split("@")[0],license_id:s.license_id||"DOC2026",institution:s.institution||"PathoAI Clinical Center",role:"Pathologist"};return localStorage.setItem("pathoai_active_user",JSON.stringify(o)),{message:"Login successful",user:o}}if(L()||i.status===405||i.message.includes("405")||i.message.includes("Failed to fetch")){const o={email:e,full_name:e.split("@")[0].replace("."," ").toUpperCase(),license_id:"DOC202688",institution:"PathoAI Medical Center",role:"Pathologist"};return localStorage.setItem("pathoai_active_user",JSON.stringify(o)),{message:"Demo Login Successful",user:o}}throw i}},register:async e=>{try{return await f("/auth/register",{method:"POST",body:JSON.stringify(e)})}catch(t){if(L()||t.status===405||t.message.includes("405")||t.message.includes("Failed to fetch"))return ge(e),{message:"User registered successfully (Local Clinical Session)",user:{email:e.email,full_name:e.full_name,license_id:e.license_id,institution:e.institution}};throw t}},forgotPassword:async e=>{try{return await f("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})})}catch(t){if(L()||t.status===405||t.message.includes("405"))return window._pathoai_demo_otp="123456",{message:"OTP sent successfully. (Demo OTP: 123456)"};throw t}},verifyOTPReset:async(e,t,a)=>{try{return await f("/auth/verify-otp-reset",{method:"POST",body:JSON.stringify({email:e,otp:t,new_password:a})})}catch(i){if(L()||i.status===405||i.message.includes("405")){const n=F(),s=n.find(o=>o.email.toLowerCase()===e.toLowerCase());return s&&(s.password=a,localStorage.setItem("pathoai_users",JSON.stringify(n))),{message:"Password reset successfully"}}throw i}},logout:async()=>{localStorage.removeItem("pathoai_active_user"),localStorage.removeItem("pathoai_logged_in");try{await f("/auth/logout",{method:"POST"})}catch{}},updateProfile:async e=>{try{return await f("/auth/me",{method:"PUT",body:JSON.stringify(e)})}catch{const i={...JSON.parse(localStorage.getItem("pathoai_active_user")||"{}"),...e};return localStorage.setItem("pathoai_active_user",JSON.stringify(i)),i}},changePassword:(e,t)=>f("/auth/change-password",{method:"POST",body:JSON.stringify({current_password:e,new_password:t})}),getDashboard:async()=>{try{return await f("/dashboard")}catch{const t=await N();return{total_patients:t.length,analyzed_cases:t.filter(a=>a.status!=="Pending").length,high_risk_cases:t.filter(a=>(a.risk_score||0)>=65).length,recent_cases:t.slice(0,5)}}},getPatients:async e=>{try{const t=e?`/patients/?q=${encodeURIComponent(e)}`:"/patients/",a=await f(t);if(Array.isArray(a)){await P();for(const i of a)await B(i);return a}}catch{}return await N()},getPatientById:async e=>{try{return await f(`/patients/${e}`)}catch{return await le(e)}},createPatient:async e=>{try{const t=await f("/patients/",{method:"POST",body:JSON.stringify(e)});return await P(),await B(t),t}catch{await P();const a={...e,patient_uid:e.patient_uid||`PT-${Date.now().toString().slice(-4)}`,created_at:new Date().toISOString()};return await B(a)}},updatePatient:async(e,t)=>{try{const a=await f(`/patients/${e}`,{method:"PUT",body:JSON.stringify(t)});return await P(),await B(a),a}catch{return await P(),await B({...t,id:Number(e)||e})}},deletePatient:async e=>{try{await f(`/patients/${e}`,{method:"DELETE"})}catch{}return await P(),await de(e),null},getSettings:async()=>{try{return await f("/settings")}catch{return{ai_threshold:await O("ai_threshold")||.65,gradcam_layer:await O("gradcam_layer")||"features.7.2",theme:await O("theme")||"dark"}}},saveSettings:async e=>{try{return await f("/settings",{method:"PUT",body:JSON.stringify(e)})}catch{for(const[t,a]of Object.entries(e))await ce(t,a);return e}},getActivities:async()=>{try{return await f("/activity")}catch{return[{title:"Biopsy Analyzed",description:"Oral Squamous Cell Carcinoma detected (88% confidence)",time:"10m ago",icon:"fas fa-microscope"},{title:"New Case Added",description:"Patient Sunita Reddy registered for buccal screening",time:"1h ago",icon:"fas fa-user-plus"}]}},getNotifications:async()=>{try{return await f("/notifications")}catch{return[]}},markNotificationRead:e=>f(`/notifications/${e}/read`,{method:"PUT"}).catch(()=>null),logSearch:e=>f("/search/log",{method:"POST",body:JSON.stringify({query:e})}).catch(()=>null),getPredictionHistory:async(e=null)=>{try{const t=e?`/history/${e}`:"/history";return await f(t)}catch{return[]}},analyzeSlide:async(e,t=null,a=null)=>{try{const i=new FormData;if(e instanceof File||e instanceof Blob)i.append("file",e);else{const n=new Blob(["dummy"],{type:"image/png"});i.append("file",n,"sample_slide.png")}return t&&i.append("patient_id",t),a&&i.append("user_id",a),await f("/predict",{method:"POST",body:i})}catch{return{prediction_id:`PRED-${Date.now()}`,status:"Analyzed",diagnosis:"Oral Squamous Cell Carcinoma (OSCC)",risk_score:87.5,confidence:.942,class_name:"OSCC - High Grade",tissue_type:"Epithelial Biopsy",metrics:{nuclear_atypia:"Severe",keratinization:"Prominent Keratin Pearls",mitotic_figures:"High (8/10 HPF)",stromal_invasion:"Positive"},cam_overlay:null}}},healthCheck:()=>f("/health").catch(()=>({status:"offline-preview"}))};function me(){return`
  ...
  `}function be(){window.PathoApp.handleSearch=t=>{w({searchQuery:t})},window.PathoApp.toggleSidebar=()=>{const t=document.querySelector(".sidebar");t&&t.classList.toggle("open")},window.PathoApp.toggleTheme=()=>{const a=(document.documentElement.getAttribute("data-theme")||"dark")==="dark"?"light":"dark";document.documentElement.setAttribute("data-theme",a),localStorage.setItem("pathoai_theme",a);const i=document.getElementById("theme-icon"),n=document.getElementById("theme-label");i&&(i.textContent=a==="dark"?"☀️":"🌙"),n&&(n.textContent=a==="dark"?"Day Mode":"Night Mode")};const e=localStorage.getItem("pathoai_theme")||"dark";document.documentElement.setAttribute("data-theme",e)}function we(){const{activePage:e,patients:t}=u(),a=t.filter(s=>s.status==="Pending").length,i=t.filter(s=>(s.risk_score||0)>=65).length;return`
    <aside class="sidebar">
  ...
  `}async function j(){try{const e=await y.getDashboard(),{stats:t,recent_patients:a}=e||{},{searchQuery:i,patients:n}=u();if(t){T("kpi-total",t.total_patients||0),T("kpi-analyzed",t.analyzed_patients||0),T("kpi-highrisk",t.high_risk_patients||0),T("kpi-pending",t.pending_patients||0);const l=t.total_patients||0,r=t.analyzed_patients||0,d=l>0?Math.round(r/l*100):0,c=document.getElementById("ring-circle"),m=document.getElementById("ring-pct");if(c){const p=Math.round(d/100*251);c.setAttribute("stroke-dasharray",`${p} 251`)}m&&(m.textContent=`${d}%`)}let s=a&&a.length>0?a:n;if(i&&s){const l=i.toLowerCase();s=s.filter(r=>(r.name||"").toLowerCase().includes(l)||(r.biopsy_site||"").toLowerCase().includes(l)||(r.diagnosis||"").toLowerCase().includes(l))}const o=document.getElementById("dash-patient-list");o&&(!s||s.length===0?o.innerHTML=Z():o.innerHTML=s.slice(0,6).map(Q).join(""))}catch(e){console.warn("Dashboard /dashboard endpoint failed:",e.message)}}function T(e,t){const a=document.getElementById(e);if(!a)return;const i=parseInt(a.textContent)||0,n=900,s=performance.now(),o=l=>{const r=Math.min((l-s)/n,1),d=1-Math.pow(1-r,3);a.textContent=Math.round(i+(t-i)*d),r<1&&requestAnimationFrame(o)};requestAnimationFrame(o)}function Pe(){var p;const{patients:e,searchQuery:t}=u(),a=e.filter(g=>{if(!t)return!0;const h=t.toLowerCase();return g.name.toLowerCase().includes(h)||(g.biopsy_site||"").toLowerCase().includes(h)||(g.diagnosis||"").toLowerCase().includes(h)}),i=e.length,n=e.filter(g=>g.status!=="Pending").length,s=e.filter(g=>(g.risk_score||0)>=65).length,o=e.filter(g=>g.status==="Pending").length,l=[...a].slice(0,6),r=(g,h)=>g.map(b=>`
  ...
  `}function xe(e){const t=e.filter(l=>(l.risk_score||0)>=65).length,a=e.filter(l=>(l.risk_score||0)>=40&&(l.risk_score||0)<65).length,i=e.filter(l=>(l.risk_score||0)>0&&(l.risk_score||0)<40).length,n=e.filter(l=>!l.risk_score||l.status==="Pending").length,s=Math.max(e.length,1),o=(l,r,d,c,m)=>`
  ...
  `}function ke(){const e=u(),t=!!e.currentUser,a=e.patients&&e.patients.length>0;return[{name:"FastAPI Backend",status:t?"online":"offline",color:t?"#10b981":"#f43f5e"},{name:"MongoDB Database",status:a?"online":"mock",color:a?"#10b981":"#f59e0b"},{name:"MIL AI Engine",status:"ready",color:"#38bdf8"},{name:"Image Processor",status:"ready",color:"#38bdf8"}].map(n=>`
  ...
  `}function Ie(){const{patients:e,searchQuery:t,riskFilter:a}=u();let i=e.filter(n=>{if(a==="high"&&(n.risk_score||0)<65||a==="moderate"&&((n.risk_score||0)<40||(n.risk_score||0)>=65)||a==="low"&&(n.risk_score||0)>=40||a==="pending"&&n.status!=="Pending"||a==="analyzed"&&n.status==="Pending")return!1;if(!t)return!0;const s=t.toLowerCase();return(n.name||"").toLowerCase().includes(s)||(n.patient_uid||"").toLowerCase().includes(s)||(n.biopsy_site||"").toLowerCase().includes(s)||(n.tissue_type||"").toLowerCase().includes(s)||(n.diagnosis||"").toLowerCase().includes(s)});return`
    <div id="page-patients" class="page-content">
      ...
      <div class="patient-grid" id="full-patient-list">
        ${i.length>0?i.map(n=>Se(n)).join(""):Ae()}
      </div>
  ...
  `}function Se(e){const t=e.risk_score>=65?"badge-high":e.risk_score>=40?"badge-mod":e.status==="Pending"?"badge-pending":"badge-low";return`
  ...
  `}function _e(){window.PathoApp.setRiskFilter=e=>{w({riskFilter:e})},window.PathoApp.openAddPatientModal=()=>{const e=document.getElementById("modal-add-patient");e&&e.classList.add("active")},window.PathoApp.closeAddPatientModal=()=>{const e=document.getElementById("modal-add-patient");e&&e.classList.remove("active")},window.PathoApp.handleAddPatient=async()=>{const e=document.getElementById("p-name").value.trim(),t=document.getElementById("p-age").value,a=document.getElementById("p-gender").value,i=document.getElementById("p-site").value.trim(),n=document.getElementById("p-notes").value.trim();if(!e||!t||!i){v("Please fill in name, age, and biopsy site.","error");return}const s=document.querySelector('#form-add-patient button[type="submit"]');s&&(s.disabled=!0,s.textContent="Saving…");try{const o=await y.createPatient({name:e,age:parseInt(t),gender:a,biopsy_site:i,notes:n}),l=await y.getPatients();w({patients:l});const r=document.getElementById("full-patient-list");r&&l.length>0&&(r.innerHTML=l.map(d=>{const c=(d.risk_score||0)>=65?"badge-high":(d.risk_score||0)>=40?"badge-mod":d.status==="Pending"?"badge-pending":"badge-low";return`<div class="patient-card" onclick="window.PathoApp.openPatientDetails('${d.id||d.patient_uid||""}')">
          ...
          </div>`}).join("")),v(`Patient case for ${e} registered & saved to MongoDB!`,"success"),window.PathoApp.closeAddPatientModal(),["p-name","p-age","p-site","p-notes"].forEach(d=>{const c=document.getElementById(d);c&&(c.value="")})}catch(o){v(o.message||"Failed to create patient record","error")}finally{s&&(s.disabled=!1,s.textContent="Register Patient Case")}},window.PathoApp.openPatientDetails=e=>{const t=u().patients.find(a=>String(a.id)===String(e)||String(a._id)===String(e)||String(a.patient_uid)===String(e));t?(R(t),_("page-inspector")):v("Patient record not found.","error")}}function X(e){if(!e)return"/pathology_slide_sample.png";if(e.startsWith("data:")||e.startsWith("http"))return e;if(e.startsWith("/uploads/")||e.startsWith("uploads/")){const t=e.startsWith("/")?e:`/${e}`;return`${U()}${t}`}return e.includes("pathology_slide_sample.png")?"/pathology_slide_sample.png":e}let z=null;function K(){const{activePatient:e,inspector:t,patients:a}=u(),i=e||a[0]||null;if(!i)return`
    ...
    <div id="page-inspector" class="page-content">
      ...
      <div style="background:var(--card-bg); border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:1.2rem 1.5rem; margin-bottom:1.5rem; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:1rem;">
        <div style="display:flex; align-items:center; gap:1.2rem;">
          <img src="${X(i.localImageUrl||i.image_url)||"/pathology_slide_sample.png"}" 
  ...
  `}function ee(){const e=document.getElementById("slide-canvas");if(!e)return;const t=e.getContext("2d"),a=e.parentElement.getBoundingClientRect();e.width=a.width||800,e.height=a.height||500;const{activePatient:i,patients:n}=u(),s=i||n[0];if(!s)return;const o=new Image,l=X(s.localImageUrl||s.image_url)||"/pathology_slide_sample.png";o.src=l,o.onload=()=>{z=o,r()},o.onerror=()=>{o.src!==window.location.origin+"/pathology_slide_sample.png"&&o.src!=="/pathology_slide_sample.png"&&(o.src="/pathology_slide_sample.png")};function r(){if(!t||!z)return;const{inspector:p}=u(),g=p.mode||"overlay";t.clearRect(0,0,e.width,e.height),t.save();const h=e.width/2+(p.panX||0),b=e.height/2+(p.panY||0),E=p.zoom||1,x=z.width*E*.45,S=z.height*E*.45;g==="original"||g==="overlay"?t.drawImage(z,h-x/2,b-S/2,x,S):g==="mask"&&(t.fillStyle="#060d1d",t.fillRect(h-x/2,b-S/2,x,S)),(g==="mask"||g==="overlay")&&p.showHeatmap!==!1&&(t.fillStyle="rgba(244, 63, 94, 0.35)",t.beginPath(),t.arc(h,b-20,x*.22,0,Math.PI*2),t.fill(),t.fillStyle="rgba(245, 158, 11, 0.28)",t.beginPath(),t.arc(h+x*.2,b+30,x*.18,0,Math.PI*2),t.fill()),p.showBoundingBoxes!==!1&&(t.strokeStyle="#38bdf8",t.lineWidth=2,t.strokeRect(h-80,b-60,160,120),t.fillStyle="#38bdf8",t.font='600 12px "Plus Jakarta Sans", sans-serif',t.fillText(`ROI Tile #42 · ${s.diagnosis||"Oral Dysplasia"}`,h-78,b-68)),t.restore()}let d=!1,c,m;e.onmousedown=p=>{d=!0,c=p.clientX,m=p.clientY},window.onmousemove=p=>{if(!d)return;const g=p.clientX-c,h=p.clientY-m;c=p.clientX,m=p.clientY;const{inspector:b}=u();b.panX=(b.panX||0)+g,b.panY=(b.panY||0)+h,r()},window.onmouseup=()=>{d=!1},window.PathoApp.setInspectorMode=p=>{const{inspector:g}=u();g.mode=p,w({inspector:g}),r()},window.PathoApp.zoomCanvas=p=>{const{inspector:g}=u();g.zoom=Math.max(.5,Math.min(4,(g.zoom||1)+p));const h=document.getElementById("zoom-level-text");h&&(h.textContent=`${Math.round(g.zoom*10)}x`),r()},window.PathoApp.toggleHeatmap=()=>{const{inspector:p}=u();p.showHeatmap=!p.showHeatmap,r()},window.PathoApp.toggleBoundingBoxes=()=>{const{inspector:p}=u();p.showBoundingBoxes=!p.showBoundingBoxes,r()},window.PathoApp.resetCanvas=()=>{const{inspector:p}=u();p.zoom=1,p.panX=0,p.panY=0;const g=document.getElementById("zoom-level-text");g&&(g.textContent="10x"),r()}}let D=null,A=null;function Ce(){const{patients:e}=u();return`
  ...
  `}function Ee(){setTimeout(()=>{const e=document.getElementById("drop-zone");e&&(e.addEventListener("dragover",t=>{t.preventDefault(),e.style.borderColor="var(--primary)",e.style.background="rgba(56,189,248,0.12)"}),e.addEventListener("dragleave",()=>{e.style.borderColor="var(--primary-light)",e.style.background="rgba(6,13,29,0.5)"}),e.addEventListener("drop",t=>{t.preventDefault(),e.style.borderColor="var(--primary-light)",e.style.background="rgba(6,13,29,0.5)",t.dataTransfer.files&&t.dataTransfer.files[0]&&window.PathoApp.handleFileSelected(t.dataTransfer.files[0])}))},100),window.PathoApp.handleFileSelected=e=>{if(!e)return;D=e;const t=new FileReader;t.onload=a=>{A=a.target.result;const i=document.getElementById("preview-img-src"),n=document.getElementById("preview-filename"),s=document.getElementById("preview-filesize"),o=document.getElementById("file-preview-card");i&&(i.src=A),n&&(n.textContent=e.name),s&&(s.textContent=`${(e.size/(1024*1024)).toFixed(2)} MB · Uploaded Image Ready for Scan`),o&&(o.style.display="block")},t.readAsDataURL(e)},window.PathoApp.clearSelectedFile=()=>{D=null,A=null;const e=document.getElementById("file-preview-card"),t=document.getElementById("slide-file-input");e&&(e.style.display="none"),t&&(t.value="")},window.PathoApp.runPipelineAnalysis=async()=>{var n;const e=document.getElementById("pipeline-stepper"),t=document.getElementById("btn-run-pipeline"),a=((n=document.getElementById("select-patient-case"))==null?void 0:n.value)||null;if(!a){v("Please select a patient case before running analysis.","error");return}if(!D){v("Please upload a pathology slide image first.","error");return}e&&(e.style.display="block"),t&&(t.disabled=!0,t.innerHTML='<i class="fas fa-spinner fa-spin"></i> Running Deep MIL Analysis…');const i=(s,o,l=!1)=>setTimeout(()=>{const r=document.getElementById(s);if(!r)return;r.style.opacity="1";const d=r.querySelector("i");d&&(l?(d.className="fas fa-check-circle",d.style.color="var(--success)"):(d.className="fas fa-spinner fa-spin",d.style.color="var(--primary-light)"))},o);i("step-2",700),i("step-3",1400),i("step-4",2100),i("step-4",2800,!0);try{await new Promise(d=>setTimeout(d,2900));const{currentUser:s}=u(),o=await y.analyzeSlide(D,a,(s==null?void 0:s.id)||null);v("✓ Deep MIL Analysis complete — results updated!","success");const l=await y.getPatients();let r=l.find(d=>String(d.id)===String(a)||String(d.patient_uid)===String(a));o?(r||(r={id:a}),(o.prediction||o.diagnosis)&&(r.diagnosis=o.prediction||o.diagnosis),(o.risk_score!==void 0||o.score!==void 0)&&(r.risk_score=o.risk_score??o.score),o.confidence!==void 0&&(r.confidence=o.confidence),o.notes&&(r.notes=o.notes),(o.heatmap_url||o.file_url)&&(r.image_url=o.heatmap_url||o.file_url),A&&(r.localImageUrl=A)):A&&r&&(r.image_url=A),w({patients:l}),r?R(r):l.length>0&&R(l[0]),setTimeout(()=>_("page-inspector"),500)}catch(s){v(`Analysis failed: ${s.message}`,"error"),console.error("[AnalysisPipeline] Error:",s)}finally{t&&(t.disabled=!1,t.innerHTML='<i class="fas fa-play"></i> Execute Deep MIL Analysis Pipeline')}}}function $e(e){if(!e)return"/pathology_slide_sample.png";if(e.startsWith("data:")||e.startsWith("http"))return e;if(e.startsWith("/uploads/")||e.startsWith("uploads/")){const t=e.startsWith("/")?e:`/${e}`;return`${U()}${t}`}return e.includes("pathology_slide_sample.png")?"/pathology_slide_sample.png":e}function te(){const{activePatient:e,currentUser:t,patients:a}=u(),i=e||(a&&a.length>0?a[0]:null);if(!i)return`
  ...
  `}function Be(){const{currentUser:e}=u(),t=e?e.full_name||e.email.split("@")[0]:"Clinical Pathologist",a=e?e.email:"No user logged in",i=e&&e.license_id||"",n=e&&e.institution||"",s=e&&e.role?`Consultant ${e.role.charAt(0).toUpperCase()+e.role.slice(1)}`:"Consultant Pathologist";return`
  ...
  `}function ze(){window.PathoApp._togglePwd=(e,t)=>{const a=document.getElementById(e);if(!a)return;const i=a.type==="password";a.type=i?"text":"password";const n=t.querySelector("i");n&&(n.className=i?"fas fa-eye-slash":"fas fa-eye")},window.PathoApp._updatePwdStrength=(e,t)=>{const a=document.getElementById(t);if(!a)return;let i=0;e.length>=8&&i++,e.length>=12&&i++,/[A-Z]/.test(e)&&i++,/[0-9]/.test(e)&&i++,/[^A-Za-z0-9]/.test(e)&&i++;const n=["#ef4444","#f97316","#eab308","#22c55e","#16a34a"],s=["20%","40%","60%","80%","100%"];a.style.background=n[i-1]||"#374151",a.style.width=i>0?s[i-1]:"0"},window.PathoApp.saveProfileSettings=async()=>{const e=document.getElementById("edit-prof-name"),t=document.getElementById("edit-prof-lic"),a=document.getElementById("edit-prof-inst"),i=document.getElementById("profile-save-error"),n=document.getElementById("profile-save-success"),s=document.getElementById("btn-save-profile"),o=document.getElementById("save-profile-btn-text");i&&(i.style.display="none",i.textContent=""),n&&(n.style.display="none",n.textContent="");const l=e?e.value.trim():"",r=t?t.value.trim():"",d=a?a.value.trim():"";if(!l||l.length<2){i&&(i.textContent="Full name must be at least 2 characters.",i.style.display="block");return}s&&(s.disabled=!0),o&&(o.textContent="Saving…");try{const c=await y.updateProfile({full_name:l,license_id:r,institution:d}),{currentUser:m}=u(),p={...m,full_name:c.full_name||l,institution:c.institution||d,license_id:c.license_id||r};w({currentUser:p});const g=document.getElementById("prof-name"),h=document.getElementById("prof-institution"),b=document.getElementById("prof-id"),E=document.getElementById("prof-avatar"),x=document.getElementById("hdr-name"),S=document.getElementById("hdr-avatar");g&&(g.textContent=c.full_name||l),h&&(h.textContent=c.institution||d||"PathoAI Medical Center"),b&&(b.textContent=`License: ${c.license_id||r||"Not specified"}`);const q=`https://ui-avatars.com/api/?name=${encodeURIComponent(c.full_name||l)}&background=38bdf8&color=0b172e&bold=true&size=128`;E&&(E.src=q),x&&(x.textContent=c.full_name||l),S&&(S.src=q),n&&(n.textContent="✓ Profile saved to MongoDB successfully!",n.style.display="block"),v("Profile credentials saved to MongoDB!","success")}catch(c){const m=c.message||"Failed to save profile. Please try again.";i&&(i.textContent=m,i.style.display="block"),v(m,"error")}finally{s&&(s.disabled=!1),o&&(o.textContent="Save Profile")}},window.PathoApp.changePassword=async()=>{const e=document.getElementById("chg-current-pass"),t=document.getElementById("chg-new-pass"),a=document.getElementById("chg-confirm-pass"),i=document.getElementById("chgpwd-error"),n=document.getElementById("chgpwd-success"),s=document.getElementById("btn-change-pwd"),o=document.getElementById("change-pwd-btn-text");i&&(i.style.display="none",i.textContent=""),n&&(n.style.display="none",n.textContent="");const l=e?e.value:"",r=t?t.value:"",d=a?a.value:"";if(!l){i&&(i.textContent="Please enter your current password.",i.style.display="block");return}if(!r||r.length<8){i&&(i.textContent="New password must be at least 8 characters.",i.style.display="block");return}if(r!==d){i&&(i.textContent="New passwords do not match.",i.style.display="block");return}s&&(s.disabled=!0),o&&(o.textContent="Changing…");try{const c=await y.changePassword(l,r);n&&(n.textContent=`✓ ${c.msg||"Password changed successfully."}`,n.style.display="block"),v("Password changed successfully!","success"),e&&(e.value=""),t&&(t.value=""),a&&(a.value="");const m=document.getElementById("chg-strength");m&&(m.style.background="",m.style.width="0")}catch(c){const m=c.message||"Failed to change password. Please check your current password.";i&&(i.textContent=m,i.style.display="block")}finally{s&&(s.disabled=!1),o&&(o.textContent="Change Password")}},window.PathoApp.exportDatabaseBackup=async()=>{try{await pe(),v("Database JSON backup downloaded!","success")}catch(e){v("Database export failed: "+e.message,"error")}}}window.PathoApp=window.PathoApp||{};window.PathoApp.setPage=e=>_(e);window.PathoApp.setView=e=>k(e);window.PathoApp.showView=e=>k(e);window.PathoApp.switchPage=e=>_(e);window.PathoApp.showToast=(e,t)=>v(e,t);window.PathoApp.getState=()=>u();window.PathoApp.setState=e=>w(e);window.PathoApp.refreshDashboard=j;window.PathoApp.openPatientDetails=e=>{const{patients:t}=u(),a=t.find(i=>String(i.id)===String(e)||String(i._id)===String(e)||String(i.patient_uid)===String(e));a?(R(a),_("page-inspector")):v("Patient record not found.","error")};window.PathoApp.navigateToFilteredPatients=e=>{w({riskFilter:e}),_("page-patients")};window.showView=e=>k(e);window.switchPage=e=>_(e);window.enterApp=()=>k("view-app");window.toast=(e,t)=>v(e,t);window.qs=e=>document.querySelector(e);document.addEventListener("DOMContentLoaded",async()=>{try{await P()}catch(e){console.warn("IndexedDB Init Warning:",e)}Le(),he(),be(),_e(),Ee(),ze(),ie(Te),setTimeout(async()=>{try{const e=await y.getCurrentUser();w({currentUser:e}),localStorage.setItem("pathoai_logged_in","true");try{const t=await y.getPatients();w({patients:t})}catch(t){console.warn("Patient fetch warning:",t.message)}k("view-app"),setTimeout(j,200)}catch{localStorage.removeItem("pathoai_logged_in"),k("view-login")}},1200)});function Le(){const e=document.getElementById("app-shell");if(!e)return;e.innerHTML=`
    <!-- Ambient Backdrop -->
  ...
  `,document.querySelectorAll(".view").forEach(i=>{i.classList.remove("active"),i.style.display="none"});const t=document.getElementById("view-splash");t&&(t.classList.add("active"),t.style.display="flex"),document.querySelectorAll(".page-content").forEach(i=>{i.classList.remove("active"),i.style.display="none"});const a=document.getElementById("page-dashboard");a&&(a.classList.add("active"),a.style.display="flex"),ee()}let H=null,V=null,G=null,J=null,W=null;function Te(e){var l;const t=e.activePatient?e.activePatient.id||e.activePatient.patient_uid:null,a=t!==G;a&&(G=t);const i=e.searchQuery!==J;i&&(J=e.searchQuery);const n=e.riskFilter!==W;n&&(W=e.riskFilter),e.activeView!==H&&(H=e.activeView,document.querySelectorAll(".view").forEach(r=>{const d=r.id===e.activeView;r.classList.toggle("active",d),r.style.display=d?"flex":"none"}));const s=e.activePage!==V;if(s&&(V=e.activePage,document.querySelectorAll(".page-content").forEach(r=>{const d=r.id===e.activePage;r.classList.toggle("active",d),r.style.display=d?"flex":"none"})),(s||a||i||n)&&(e.activePage==="page-dashboard"&&j(),e.activePage==="page-patients"&&De(),e.activePage==="page-inspector"&&Re(),e.activePage==="page-upload"&&Oe(),e.activePage==="page-report"&&Me(),e.activePage==="page-profile"&&Ne()),document.querySelectorAll(".nav-item").forEach(r=>{var c,m;const d=(m=(c=r.getAttribute("onclick"))==null?void 0:c.match(/'([^']+)'/))==null?void 0:m[1];r.classList.toggle("active",d===e.activePage)}),e.activePage==="page-dashboard"){const{patients:r}=e,d=document.getElementById("kpi-total"),c=document.getElementById("kpi-analyzed"),m=document.getElementById("kpi-highrisk");d&&(d.textContent=r.length),c&&(c.textContent=r.filter(p=>p.status!=="Pending").length),m&&(m.textContent=r.filter(p=>(p.risk_score||0)>=65).length)}const o=document.getElementById("toast");if(o&&(e.toast.visible?(o.textContent=e.toast.message,o.className=`toast show ${e.toast.type}`):o.classList.remove("show")),e.currentUser){const r=document.getElementById("hdr-name"),d=document.getElementById("hdr-avatar");if(r&&(r.textContent=e.currentUser.full_name||((l=e.currentUser.email)==null?void 0:l.split("@")[0])||""),d){const c=encodeURIComponent(e.currentUser.full_name||e.currentUser.email||"U");d.src=`https://ui-avatars.com/api/?name=${c}&background=38bdf8&color=0b172e&bold=true`}}}async function De(){try{const{searchQuery:e,riskFilter:t}=u();let a;try{a=await y.getPatients(),w({patients:a})}catch(s){console.warn("Patient fetch failed, using cached state:",s.message),a=u().patients}const i=a.filter(s=>{if(t==="high"&&(s.risk_score||0)<65||t==="moderate"&&((s.risk_score||0)<40||(s.risk_score||0)>=65)||t==="low"&&(s.risk_score||0)>=40||t==="pending"&&s.status!=="Pending"||t==="analyzed"&&s.status==="Pending")return!1;if(!e)return!0;const o=e.toLowerCase();return(s.name||"").toLowerCase().includes(o)||(s.patient_uid||"").toLowerCase().includes(o)||(s.biopsy_site||"").toLowerCase().includes(o)||(s.tissue_type||"").toLowerCase().includes(o)||(s.diagnosis||"").toLowerCase().includes(o)});document.querySelectorAll("#page-patients .btn-secondary").forEach(s=>{const o=s.getAttribute("onclick")||"";if(o.includes("setRiskFilter")){const l=o.includes(`'${t}'`);s.classList.toggle("active",l),o.includes("'all'")&&(s.textContent=`All Cases (${a.length})`,l&&s.classList.add("active"))}});const n=document.getElementById("full-patient-list");n&&(i.length===0?n.innerHTML=Ue():(n.innerHTML=i.map(s=>Fe(s)).join(""),n.querySelectorAll(".patient-card").forEach(s=>{s.addEventListener("click",()=>{const o=s.dataset.pid;window.PathoApp.openPatientDetails(o)})})))}catch(e){console.warn("Patient list refresh failed:",e.message)}}function Oe(){const e=document.getElementById("select-patient-case");if(!e)return;const{patients:t}=u();e.innerHTML='<option value="">-- Create New Case or Select Patient --</option>'+t.map(a=>`<option value="${a.id||a._id||a.patient_uid}">${a.name} (${a.biopsy_site||"Oral Cavity"})</option>`).join("")}function Re(){const e=document.getElementById("page-inspector");if(!e)return;const t=e.classList.contains("active"),a=document.createElement("div");a.innerHTML=K();const i=a.firstElementChild;t&&i.classList.add("active"),e.replaceWith(i),setTimeout(ee,50)}function Me(){const e=document.getElementById("page-report");if(!e)return;const t=e.classList.contains("active"),a=document.createElement("div");a.innerHTML=te();const i=a.firstElementChild;t&&i.classList.add("active"),e.replaceWith(i)}function Ne(){const{currentUser:e}=u();if(!e)return;const t=document.getElementById("prof-name"),a=document.getElementById("prof-email"),i=document.getElementById("prof-institution"),n=document.getElementById("prof-id");t&&(t.textContent=e.full_name||""),a&&(a.textContent=e.email||""),i&&(i.textContent=e.institution||"PathoAI Medical Center"),n&&(n.textContent=`License: ${e.license_id||"Not specified"}`)}function Fe(e){const t=(e.risk_score||0)>=65?"badge-high":(e.risk_score||0)>=40?"badge-mod":e.status==="Pending"?"badge-pending":"badge-low",a=e.id||e._id||e.patient_uid||"";return`
...
//# sourceMappingURL=index-7rol1A-W.js.map
